...
<title>NewsPortal — Редактировать новость</title>
    ...
    <div class="card mx-auto shadow-lg" style="max-width: 700px;">
        ...
        <form action="NewsPortalController" method="post">
            ...
            <div class="mb-3">
                ...
                <textarea class="form-control" id="brief" name="brief" rows="3" required>${news.brief}</textarea>
            </div>

            <div class="mb-3">
                <label for="contentPath" class="form-label">Ссылка на полный текст</label>
                <input type="text" class="form-control" id="contentPath" name="contentPath" value="${news.contentPath}" required>
            </div>

            <div class="mb-3">
                <label for="status" class="form-label">Статус</label>
                <select class="form-select" id="status" name="action">
                    <option value="draft" <c:if test="${news.statusId == 2}">selected</c:if>>Черновик</option>
                    <option value="publish" <c:if test="${news.statusId == 1}">selected</c:if>>Опубликовать</option>
                </select>
            </div>

            <div class="mb-3">
                <label class="form-label">Дата публикации</label>
                <input type="text" class="form-control" value="${news.publishDate}" readonly>
            </div>
